...
char appData;  //The data received from the app (a single char)
...
void loop()
{
  ...
  while (HM10.available() > 0) {   // if HM10 sends something then read
    appData = HM10.read(); // save the data in string format
...
void configure_shot(){
  ...
  //Deleting the shot just extracting from the queue.
  shooting_queue.remove(0, pos+delimiter.length());

  //Deleting '{' and '}' from the shot string.
  shot.remove(0,1);
  shot.remove(shot.length()-1, 1);

  //Extracting the values for the shot: [shot_delay, left_right, up_down, motor_speed]
  for(int i = 0; i < 4; i++){
    pos = shot.indexOf(',');
